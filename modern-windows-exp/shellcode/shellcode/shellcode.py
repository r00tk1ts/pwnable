...
def get_cstring(data, offset):
    '''
    stretch C string(ends with NULL)
    '''
...
def get_shellcode_len(map_file):
    '''
    get length of shellcode by analyzing map file(generated by VS2015)
    '''
    try:
...
def get_shellcode_and_relocs(exe_file, shellcode_len):
    '''
    dump shellcode and relocated string from .text section.
    return a tuple of three, (shellcode, reloc, str)
    '''
    ...
    try:
        ...
        # locate in shellcode's position needed to relocate, dump these string according to the relocation in .rdata section.
        relocs = []
        addr_to_strings = {}
        ...
        for rel_data in pe.DIRECTORY_ENTRY_BASERELOC:
            for entry in rel_data.entries[:-1]:         # the last element's rvs is the base_rva (why?)
                if shellcode_start <= entry.rva < shellcode_end:
                    # reloc position is in shellcode
                    relocs.append(entry.rva - shellcode_start)      # offset related to shellcode start postion
...
def add_loader_to_shellcode(shellcode, relocs, addr_to_strings):
    if len(relocs) == 0:
        return shellcode                # no address to relocate
    ...
    #       ...
    #   shellcode_start:
    #       <shellcode>         (include offset of strX (offset is related to "here" tag))
    #   relocs:
    #       off1|off2|...       (address offset to relocate(offset is related to "here" tag))
    #       str1|str2|...
    ...
    # build last part (offX and strX).
    offset = delta + len(shellcode) + len(relocs) * 4           # offset is related to "here" tag
    ...
    # last step: fix shellcode that address to be relocated will point to the correct string
